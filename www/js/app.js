...
example.controller("SignupController", function ($scope, $firebaseAuth, $state, $location) {
    ...
    $scope.listProf = [];
        var getList = DBRoot.ref('/Professionality');
    ...
    $scope.register = function (nameUsr, phoneUsr, profUsr, emailUsr, passwordUsr, birthUsr) {
        ...
        .then(function (authData) {
            ...
            DBRoot.ref('users/' + authData.uid).set(
              {
                  Name:nameUsr,
                  Phone_No:phoneUsr,
    ...
    $scope.list = function () {
        ...
        auth.$onAuthStateChanged(function (user) {
            if (user) {
                // User logged in
                uid = user.uid;
                var starCountRef = DBRoot.ref('/users/' + uid);
        ...
        $ionicPopup.prompt(
          {
              ...
              if (result != "") {
                  ...
                  DBRoot.ref('users/' + uid + '/Todos').push({ title: result })
              }
          });
    }
});
